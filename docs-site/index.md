...
flowx 的核心心智模型是把"自我迭代"拆成三层，每层职责单一、可独立组合：
...
flowx orchestrate "把 README 里的 TODO 清单逐条实现" --repo .

# 安装团队 flow，按名字运行
flowx flows install /path/to/force-dev.js
flowx run force-dev --feature my-feature --repo .

# 跑自定义 flow（dry-run 不烧 API）
FLOWX_DRY_RUN=1 flowx run ./my-flow.js
```
