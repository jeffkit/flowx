...
flowcast 的使用者，很多时候本身就是一个 **AI agent**——它被丢给 flowx，要拿它来解决任务。flowx 自己的哲学就是"给生成 flow 的 AI 一份受控词汇表"；这一页把同样的思路用到**使用方 AI** 身上。
...
└─ 标准开发流（建分支→写码→审查→PR）→ flowcast run force-dev --feature <name> --repo .
