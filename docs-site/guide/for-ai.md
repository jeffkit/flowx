...
└─ 标准开发流（建分支→写码→审查→PR）→ flowx run force-dev --feature <name> --repo .
   （需先安装：flowx flows install /path/to/force-lab/flows/force-dev.js）
```
...
# 2. 目标仓能解析 flowx（L3 orchestrate 必需）
...
# 4. dry-run 冒烟（不烧 API）
FLOWX_DRY_RUN=1 flowx orchestrate "测试目标" --repo . --agent cursor-default

# 5. 真跑
flowx orchestrate "真实目标" --repo . --agent cursor-default

# 6. 中断后续跑（同一个 run-id）
flowx orchestrate "..." --run-id <run-id>
```
...
1. 手写 flow **只能** `import flowcast`（+ `util`）；需要文件/git/进程 → 走 flowx 原语，**别裸调 `child_process`/`fs`**。
